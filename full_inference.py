...
class GroundingDINOHandler:
    ...
    @staticmethod
    def extract_confidences(pred_phrases, device):
        return torch.tensor([float(p.split('(')[-1].rstrip(')')) if '(' in p else 1.0 for p in pred_phrases], device=device)

    def nms_by_confidence(self, boxes, phrases, device, iou_threshold=0.2, max_box_size=0.8):
        widths = boxes[:, 2]
        heights = boxes[:, 3]
        ...
        confidences = self.extract_confidences(phrases, device=device)
...
class CellInference:
    ...
    def process_crop(self, crop, fname, save_name):
        self.load_models()        
        ...
        # print(f"The cell is predicted as -> {class_name}")
...
class Inference:
    def __init__(self, tiff_path, results_output_dir, device, save_model_path, save_data_path,
                 slide_type, data_name, run_name, project_type, tile_width=1024, tile_height=1024):

        self.tiff_path  = tiff_path
        self.slide_type = slide_type
        save_name = "beki" if "high" in save_model_path else "baseline"
        self.results_output_dir = f"{results_output_dir}/{data_name}_{slide_type.upper()}_{save_name}"        
        self.device = device
        self.tile_width = tile_width
        self.tile_height = tile_height
        
        self.cell_inf = CellInference(        
            results_output_dir="cell_test_LBC",
            device=self.device,
            save_model_path="/home/bekhzod/Desktop/binary_train_details/new_data/saved_models_high",
            save_data_path="/home/bekhzod/Desktop/binary_train_details/new_data/saved_dls",
            data_name="cell_LBC_25_04_15",
            run_name="compare",
            slide_type="lbc",
            project_type="binary",
            im_size=64
        ...
        self.handler = GroundingDINOHandler("/home/bekhzod/Desktop/localization_models_performance/GroundingDINO/groundingdino/config/GroundingDINO_SwinT_OGC.py", 
                                    "/home/bekhzod/Desktop/localization_models_performance/weights/groundingdino_swint_ogc.pth", device=self.device)
        print(f"\nRunning GroundingDINO with {self.device}\n")

        self.model_names = ["resnet50", "rexnet_150", "resnext50_32x4d"]
        self.ckpt_paths = [ f"{save_model_path}/{data_name}_{run_name}_{model_name}_{project_type}_best_model.pth" for model_name in self.model_names ]
        with open(f"/home/bekhzod/Desktop/binary_train_details/new_data/saved_dls/cell_LBC_25_04_15_compare_binary_64_cls_names.pkl", "rb") as fp: self.classes = pickle.load(fp)
    ...
    def load_models(self):
        ...
        for i, (model_name, ckpt_name) in enumerate(zip(self.model_names, self.ckpt_paths)):
            ...
            model.eval()
            self.models_di[f"model_{i}"] = model.to(self.device)
        print(f"\nTile classification models are loaded in {time.time() - start} secs!\n")
    ...
    def get_roi(self):
        ...
        print(f"\nROI is obtained in {time.time() - start} secs!\n")
    ...
    #     print(f"\nReading tiff slide is completed in {time.time() - read_slide_start} secs!\n")
    ...
    #                 save_file_name = os.path.join(abnormal_dir, f"{fname}_{x}_{y}.jpg")
    #                 crop.save(save_file_name)
    #                 abnormal_count += 1     

    #                 # fname = os.path.splitext(os.path.basename(image_path))[0].split("_")[0]
                    
    #                 # dirname  = os.path.basename(save_file_name).split("_")[0]
    #                 # lbl_path = f"{lbls_dir}/{dirname}/{os.path.basename(image_path).replace('.jpg', '.txt')}"

    #                 image_pil, image_tensor = self.handler.load_image(save_file_name)
    #                 boxes, phrases = self.handler.get_grounding_output(image_tensor, caption = "cells", box_threshold=0.05, text_threshold=0.05)
    #                 boxes, phrases = self.handler.nms_by_confidence(boxes = boxes, phrases = phrases, device = self.device, iou_threshold=0.2, max_box_size=0.8)
    ...
    #                     self.cell_inf.process_crop(crop=crop, fname=fname, save_name = f"{fname}_crop_{i}.jpg")
    #                     # crop.save(f"{output_dir}/{fname}_crop_{i}.jpg")
    #                 print(f"\nCell classification done in {time.time() - cell_start} secs!\n")    
    def process_tiff(self):
        self.load_models()
        os.makedirs(self.results_output_dir, exist_ok=True)        
